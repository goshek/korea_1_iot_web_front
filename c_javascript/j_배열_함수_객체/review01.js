/*
    ...
    2. 할 일 수정: 완료된 할 일의 completed값을 전환(토글)
    >> 할 일의 완료 상태를 변경
...
/*
    ? 할 일의 완료 상태를 변경하는 함수
    : 주어진 'id를 가진 할 일'의 완료 상태를 전환(토글)
*/
function toggletoDo(id){
    todos= todos.map((todo)=>{
        // 순회되는 할 일(todo)의 id와 매개변수로 전달받은 di가 일치한다면
        ...
        if(todo.id=== id){
            /*
                ...
                >>스프레드 연산자(...)
                let arr1= [1,2,3];
                let arr2= [...arr1]; >>> arr의 요소만 가져와서 새로운 배열을 생성(메모리 주소가 다름)
...
/*
    ? 할 일을 삭제하는 함수
    : 주어진 id를 가진 할 일을 todos배열에서 제거
    >> 배열 내부 요소의 개수 변화O(filter)
*/
